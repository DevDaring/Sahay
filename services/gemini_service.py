...
Generate a personalized greeting that discovers interests based on hometown and end with a question:"""
    ...
    def ask_trivia_and_discover_interests(self, message: str, username: Optional[str] = None, user_id: Optional[int] = None, language: str = "English") -> str:
        ...
        try:
            ...
            intent_classification = self._classify_user_intent(message)
            is_mental_health_topic = intent_classification == 'mental_health'
            is_study_topic = intent_classification == 'study_guidance'
...
Generate a response that shows local knowledge and asks engaging, specific questions:"""
    ...
    def _classify_user_intent(self, message: str) -> str:
        """Use Gemini to classify if user is asking for mental health support, study guidance, or general chat"""
        try:
            classification_prompt = f"""Classify the user's message into one of these categories:

CATEGORIES:
1. "mental_health" - User is expressing stress, anxiety, depression, emotional distress, or seeking mental wellness support
2. "study_guidance" - User is asking for help with academic subjects, studying, learning, or educational topics
3. "general_chat" - User is having a casual conversation, discussing interests, or other topics

USER MESSAGE: "{message}"

INSTRUCTIONS:
- Be precise in classification
- If user mentions both study stress AND academic help, prioritize "mental_health"
- If user is asking for help with specific subjects (math, science, etc.), classify as "study_guidance"
- If user is just chatting about interests or general topics, classify as "general_chat"

RESPOND WITH ONLY ONE WORD: mental_health, study_guidance, or general_chat"""
            ...
            valid_classifications = ['mental_health', 'study_guidance', 'general_chat']
            if classification in valid_classifications:
                logger.info(f"Intent classified as: {classification}")
                return classification
            else:
                logger.warning(f"Invalid classification '{classification}', defaulting to general_chat")
                return 'general_chat'
