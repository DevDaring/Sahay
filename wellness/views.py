...
class WellnessHomeView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if not wellness_df.empty:
            recent_sessions = wellness_df[wellness_df['created_at'] > (timezone.now() - timedelta(days=7))]
...
class ChatView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Personalized greeting will be loaded asynchronously via JavaScript
        # This removes the 6-second delay on page load
        context.update({
            'languages': [
    ...
    def post(self, request):
        ...
        try:
            ...
            if request.user.is_authenticated:
                ...
                logger.info(f"Live chat user: {username}, user_id: {user_id}")
            else:
                logger.info("Live chat user not authenticated")
            ...
            return JsonResponse({
                ...
                'resources': [],  # Can be populated with crisis resources if needed
...
class WellnessCheckView(LoginRequiredMixin, TemplateView):
